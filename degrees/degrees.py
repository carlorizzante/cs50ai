...
def shortest_path(source, target):
    ...
    # print(f"Start: {start.state}, End: {end.state}")
    
    # Keep looping until a solution is found
    while True:
        # Limit search to {limit} nodes
        print(f"Explored: {num_explored}")
        if num_explored > limit:
            print("Limit reached.")
            return None
        ...
        for movie_id, person_id in neighbors_for_person(node.state):
            # print(f"Movie: {movie_id}, Person: {person_id}")
            if not frontier.contains_state(person_id) and person_id not in explored:
                child = Node(state=person_id, parent=node, action=movie_id)
                if (child.state == end.state):
                    solution = []
